...
def _classify_asset(row: dict[str, Any], *, desc_text: str) -> str:
    ...
    tx_type = str(row.get("transaction_type") or row.get("type") or "").strip().upper()
...
def main() -> int:
    in_path = (os.getenv("HOUSE_PTR_IN_JSON") or "artifacts/house_ptr/house_ptr_transactions.json").strip()
    out_dir = (os.getenv("HOUSE_PTR_CLEAN_OUT_DIR") or "artifacts/house_ptr_clean").strip() or "artifacts/house_ptr_clean"
    out_path = os.path.join(out_dir, "house_ptr_transactions_clean.json")
    summary_path = os.path.join(out_dir, "house_ptr_clean_summary.json")
